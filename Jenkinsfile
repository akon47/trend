        ...
        stage('Docker Run') {
            steps {
                ...
                sshagent(credentials: ['ssh']) {
                    ...
                    sh "ssh -o StrictHostKeyChecking=no kimhwan@10.10.10.110 'docker ps -q -a --filter name=${APP_NAME} | grep -q . && docker rm -f \$(docker ps -aq --filter name=${APP_NAME}) || true'"
                    sh "ssh -o StrictHostKeyChecking=no kimhwan@10.10.10.110 'docker run -d --restart always --name ${APP_NAME}'"
